...
def input_with_search(column_list):
    ...
    try:
        while True:
            ...
            # print(f"key: {key}")
            if len(user_input) >= 2:
                suggestions = sorted(list(set([x for x in column_list if user_input.upper() in x.upper() and user_input])))

                if suggestions:
                    # print the suggestions over three columns starting with the first column in alphabetical order.
                    # restrict to 30 chars per column.
                    # print(f"suggestions: {suggestions}")
                    suggestion_number = 0
                    print()
                    for i in range(0, len(suggestions), 3):
                        if i + 2 < len(suggestions):
                            print(f"{suggestions[i][:25]:<30}{suggestions[i + 1][:25]:<30}{suggestions[i + 2][:25]:<30}")
                        elif i + 1 < len(suggestions):
                            print(f"{suggestions[i][:25]:<30}{suggestions[i + 1][:25]:<30}")
                        else:
                            print(f"{suggestions[i][:25]:<30}")
            ...
            if key == b'\x08':  # backspace key
                ...
                return user_input
                break
            elif suggestions and len(user_input) > 2 and key == b'\t':  # tab key
                while key == b'\t':
...
def print_filtered_entries(entries, date_filter_from, date_filter_to, csv=False):
    ...
    for i in entries:
        ...
        if entry_date >= date_filter_from and entry_date < date_filter_to and i.get("details").get(
                "description").upper() != "EOD":
            ...
            print(
                f"{str(i.get('details').get('from')):<15}{str(i.get('details').get('to')):<15}{str(i.get('details').get('duration')):<15}{str(round(float(i.get('details').get('duration_decimal')), 2)).replace('.', ','):<20}{i.get('details').get('description')}")
...
def main():
    ...
    while True:
        msg_entry = "Press any key to add entries, s to show entries from the log, q to quit: "
        ...
        while user_choice != 's':
            entry = input_with_search(description_list)
            if not entry:
                break
            add_joblog_entry(jobb_logg_file_name, entry)
